let highlightSwiper = new Swiper(".highlightSwiper", {
  ...
  // Hightlight
  let currentHighlightDay = 3;
  $(".tc-highlight-tab").click(function () {
    $(".tc-highlight-tab").removeClass("hack-highlight-tab-active");
    $(this).addClass("hack-highlight-tab-active");
  
    if ($(this).attr("id") === "highlight-tab-day-1") {
      if (currentHighlightDay != 1) {
        currentHighlightDay = 1;
        $(".tc-highlight-tab").addClass("pointer-events-none");
        $(".tc-highlight-content").fadeOut();
        setTimeout(() => {
          $("#highlight-content-day-1").fadeIn();
          $(".tc-highlight-tab").removeClass("pointer-events-none");
        }, 350);
      }
    } else if ($(this).attr("id") === "highlight-tab-day-2") {
      if (currentHighlightDay != 2) {
        currentHighlightDay = 2;
        $(".tc-highlight-tab").addClass("pointer-events-none");
        $(".tc-highlight-content").fadeOut();
        setTimeout(() => {
          $("#highlight-content-day-2").fadeIn();
          $(".tc-highlight-tab").removeClass("pointer-events-none");
        }, 350);
      }
    } else if ($(this).attr("id") === "highlight-tab-day-3") {
        if (currentHighlightDay != 3) {
          currentHighlightDay = 3;
          $(".tc-highlight-tab").addClass("pointer-events-none");
          $(".tc-highlight-content").fadeOut();
          setTimeout(() => {
            $("#highlight-content-day-3").fadeIn();
            $(".tc-highlight-tab").removeClass("pointer-events-none");
          }, 350);
        }
      } else if ($(this).attr("id") === "highlight-tab-day-4") {
        if (currentHighlightDay != 4) {
          currentHighlightDay = 4;
          $(".tc-highlight-tab").addClass("pointer-events-none");
          $(".tc-highlight-content").fadeOut();
          setTimeout(() => {
            $("#highlight-content-day-4").fadeIn();
            $(".tc-highlight-tab").removeClass("pointer-events-none");
          }, 350);
        }
      } else if ($(this).attr("id") === "highlight-tab-day-5") {
        if (currentHighlightDay != 5) {
          currentHighlightDay = 5;
          $(".tc-highlight-tab").addClass("pointer-events-none");
          $(".tc-highlight-content").fadeOut();
          setTimeout(() => {
            $("#highlight-content-day-5").fadeIn();
            $(".tc-highlight-tab").removeClass("pointer-events-none");
          }, 350);
        }
      }
      else {
      if (currentHighlightDay != 6) {
        currentHighlightDay = 6;
